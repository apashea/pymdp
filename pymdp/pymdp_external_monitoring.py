...
def infer_states_info_monitoring(  
    ...
    def spm_dot(A, B, factors=None):  
        """FIXED: Special matrix multiplication for pymdp object arrays"""  
        if factors is None:  
            return A @ B  
        else:  
            result = A.copy()  
    ...
    def get_joint_likelihood_seq(A, obs_seq, num_states):  
        """FIXED: Compute joint likelihood sequence for observations"""  
        num_modalities = len(A)  
        lh_seq = obj_array(len(obs_seq))  
          
        for t, obs_t in enumerate(obs_seq):  
            # Initialize joint likelihood with proper shape  
            ...
            for m in range(num_modalities):  
                ...
                # Extract likelihood for this modality and multiply  
    ...
    def get_expected_states(qs, B, action):  
        ...
        for f in range(num_factors):  
            if action is not None:  
                ...
                expected_states[f] = B[f][:, :, action_idx].T @ qs[f]  
            else:  
                expected_states[f] = qs[f]  
